  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$calc_CV, {
        ...
        ## Download Biomass map if necessary
        download_agb(
        ...
        rv$CV_model$santoro_tile <- get_santoro_tile(rv$CV_model$sf_aoi)
        ...
        ## + + Read AGB map ----
        rv$CV_model$rs_avitabile <- terra::rast(
          x = file.path(rv$CV_model$file_path, "Avitabile_AGB_Map/Avitabile_AGB_Map.tif")
          ) %>%
          terra::crop(
            )
